...
Using historical flight performance data, this project builds models to classify flights into three delay categories (`not_delayed`, `short_delay`, `long_delay`) and evaluates which model produces the most trustworthy probability estimates. The best calibrated model was then used to simulate delay-risk across different scheduled departure times for a specific flight, demonstrating how the model could support real scheduling decisions.
...
Each folder contains both a `.Rmd` (source code) and rendered `.pdf` report/presentation. The PDFs are the easiest way to review the work directly on GitHub; the `.Rmd` files contain the underlying R code.
...
- Compares calibration across all three models; Gradient Boosting was selected as the final model based on calibration quality (i.e., whether a predicted 30% delay probability actually corresponds to a ~30% observed delay rate)
...
- Discusses data limitations and outlines future directions (additional features such as aircraft type, weather, and extending the analysis to more airports)
